 ...
 */
function removeLeadingWhitespaces(v) {
  return v.trimStart();
}
 ...
 */
function removeFirstOccurrences(str, v) {
  return str.indexOf(v) === -1
    ? str
    : str.slice(0, str.indexOf(v)) + str.slice(str.indexOf(v) + v.length);
}
 ...
 */
function removeLastOccurrences(str, v) {
  return str.lastIndexOf(v) === -1
    ? str
    : str.slice(0, str.lastIndexOf(v)) +
        str.slice(str.lastIndexOf(v) + v.length);
}
 ...
 */
function getStringFromTemplate(fN, lN) {
  return `Hello, ${fN} ${lN}!`;
}
